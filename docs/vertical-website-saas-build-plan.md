...
- You're a **data processor** for clients — keep a **DPA** template + sub-processor list (Vercel, Supabase, Cloudflare, Stripe, booking/ordering providers).
...
**Capacity — and why the menu editor matters.** Concierge labour is your bottleneck, and **menus are the highest-frequency edit in any vertical.** Track minutes/edit and edits/client/month. The menu self-serve screen (Phase 4+) is what keeps restaurants profitable as you scale — prioritise it over a general editor.
...
- Render that restaurant's homepage entirely from its DB row — prove data → page end to end.
