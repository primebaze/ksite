  ...
  return (
    <div>
      ...
      <p className="max-w-2xl text-sm text-ink/50">
        ...
        <span className="text-ink">{tenant.subdomain}.kovasite.com</span> as soon as your subscription is active.
      ...
      {canceled && <p className="mt-4 rounded-lg border border-ink/10 bg-ink/[0.03] px-4 py-3 text-sm text-ink/60">Checkout canceled. You can subscribe whenever you&apos;re ready.</p>}
      ...
      <div className="mt-8 grid max-w-3xl gap-6 sm:grid-cols-2">
        {OPTIONS.map((o) => (
          <div
            key={o.period}
            className={`relative rounded-2xl border p-8 ${o.highlight ? "border-emerald-400/40 bg-gradient-to-b from-emerald-400/[0.08] to-transparent" : "border-ink/10 bg-ink/[0.02]"}`}
          >
            {o.highlight && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-emerald-400 px-3 py-1 text-xs font-semibold text-neutral-950">{PLAN.yearlyHook}</span>
            )}
            <h2 className="text-lg font-semibold">{o.label}</h2>
            <p className="mt-1 text-sm text-ink/50">{o.note}</p>
            <p className="mt-5">
              <span className="text-4xl font-bold tracking-tight">{o.price}</span>
              <span className="text-ink/40">{o.unit}</span>
            </p>
            <form action={startCheckout} className="mt-6">
              <input type="hidden" name="period" value={o.period} />
              <SubmitButton
                className={`w-full rounded-lg py-2.5 text-sm font-semibold transition ${o.highlight ? "bg-ink text-paper hover:bg-ink/90" : "border border-ink/15 text-ink hover:bg-ink/5"}`}
