...
const inputCls =
  "mt-2 w-full rounded-xl border border-ink/10 bg-ink/[0.03] px-4 py-3 text-sm text-ink placeholder-ink/25 outline-none focus:border-ink/30";
...
export default async function SetupStep({ params }: { params: Promise<{ step: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-xl">
      ...
      <p className="mt-3 text-ink/55">{step.intro}</p>
          ...
          {step.fields!.map((f) => {
            ...
            return (
              <div key={f.name}>
                ...
                {f.help && <p className="mt-1 text-xs text-ink/40">{f.help}</p>}
                {f.type === "color" ? (
                  <input name={f.name} type="color" defaultValue={v || "#111111"} className="mt-2 h-11 w-24 rounded-lg border border-ink/10 bg-transparent" />
                ) : f.type === "select" ? (
      ...
      {step.kind === "menu" && (
        <div className="mt-8">
          <h2 className="text-sm font-medium text-ink/70">Your {catalogLabel.toLowerCase()}</h2>
          <div className="mt-3 space-y-2">
            {site.catalog.length === 0 && <p className="text-sm text-ink/35">Nothing added yet. Add your first item below.</p>}
            {site.catalog.map((it) => (
              <div key={it.id} className="flex items-center justify-between rounded-xl border border-ink/10 bg-ink/[0.02] px-4 py-3">
                <div>
                  <p className="text-sm font-medium">{it.name}{it.price ? ` · ${it.price}` : ""}</p>
                  {it.description && <p className="text-xs text-ink/40">{it.description}</p>}
                </div>
                <form action={removeMenuItem}>
                  <input type="hidden" name="item_id" value={it.id} />
                  <button className="text-xs text-ink/40 hover:text-red-300">Remove</button>
                </form>
              </div>
            ))}
          </div>

          <form action={addMenuItem} className="mt-4 rounded-xl border border-dashed border-ink/15 bg-ink/[0.02] p-4">
            ...
            <SubmitButton className="mt-3 rounded-lg border border-ink/15 px-4 py-2 text-sm font-medium hover:bg-ink/5" pendingText="Adding…">+ Add item</SubmitButton>
          </form>
      ...
      {step.kind === "photos" && (
        <div className="mt-8 space-y-8">
          <div>
            ...
            <p className="mt-1 text-xs text-ink/40">A wide, real photo of your food, space or work (<span className="text-ink/60">not your logo</span>). Leave blank for a clean branded cover.</p>
            <div className="mt-3">
            ...
            {site.content.hero_image_url && (
              <form action={clearCoverPhoto} className="mt-2">
                <button className="text-xs text-ink/40 hover:text-red-300">Remove cover photo</button>
              </form>
            )}
          </div>
          <div>
            <p className="text-sm font-medium">Logo (optional)</p>
            <p className="mt-1 text-xs text-ink/40">Shown in the header. A square PNG with a transparent background works best.</p>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-ink/40">A few photos of your work, space or team.</p>
      ...
      {step.kind === "review" && (
        <div className="mt-8">
          <div className="overflow-hidden rounded-2xl border border-ink/10 bg-ink/[0.02]">
            <div className="flex items-center justify-between border-b border-ink/10 px-4 py-2 text-xs text-ink/40">
              <span>Live preview</span>
              <a href="/preview" target="_blank" rel="noreferrer" className="text-accent/90 hover:text-accent">Open full screen ↗</a>
            </div>
            <iframe src="/preview" title="Site preview" className="h-[460px] w-full bg-ink" />
          </div>

          <div className="mt-6 flex flex-wrap items-center gap-3">
            <Link href="/dashboard/publish" className="rounded-lg bg-ink px-6 py-3 text-sm font-semibold text-paper transition hover:bg-ink/90">
              Looks good, publish
            </Link>
            <Link href={`/dashboard/setup/${FIRST_STEP}`} className="rounded-lg border border-ink/15 px-5 py-3 text-sm text-ink/80 hover:bg-ink/5">
              Keep editing
            </Link>
          </div>
          {prev && (
            <div className="mt-6">
              <Link href={`/dashboard/setup/${prev.key}`} className="text-sm text-ink/45 hover:text-ink">← Back</Link>
            </div>
          )}
        </div>
      )}
...
function NavButtons({
  ...
  return (
    <div className="flex items-center justify-between pt-2">
      {prevKey ? (
        <Link href={`/dashboard/setup/${prevKey}`} className="text-sm text-ink/45 hover:text-ink">← Back</Link>
      ) : (
        <Link href="/dashboard" className="text-sm text-ink/45 hover:text-ink">← Dashboard</Link>
      )}
      {submit ? (
        <SubmitButton className="rounded-lg bg-ink px-6 py-3 text-sm font-semibold text-paper transition hover:bg-ink/90" pendingText="Saving…">{nextLabel} →</SubmitButton>
      ) : (
        <Link href={`/dashboard/setup/${nextKey}`} className="rounded-lg bg-ink px-6 py-3 text-sm font-semibold text-paper transition hover:bg-ink/90">{nextLabel} →</Link>
      )}
    </div>
  );
}
