    ...
    <div className="min-h-screen bg-black font-sans text-white antialiased">
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-72 flex-col border-r border-white/10 bg-neutral-950 px-5 py-7 lg:flex">
        ...
        <div className="mt-auto border-t border-white/10 pt-4">
          <div className="flex items-center gap-3 px-2">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/10 text-sm font-semibold">{initial}</div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium text-white/80">{user.email}</p>
              <p className="text-xs text-white/35">Operator</p>
            </div>
          </div>
          <form action={logout} className="mt-3">
            <button className="w-full rounded-xl border border-white/12 px-3 py-2 text-sm font-medium text-white/70 transition hover:bg-white/[0.06] hover:text-white">
              Sign out
            </button>
          </form>
        </div>
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-black/80 backdrop-blur-xl lg:hidden">
        <div className="flex items-center justify-between px-5 py-3.5">
          ...
          <form action={logout}>
            <button className="rounded-lg border border-white/15 px-3 py-1.5 text-sm font-medium text-white/70">Sign out</button>
          </form>
        </div>
