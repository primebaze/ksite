            ...
            <div className="space-y-16">
              {groups.map((section, gi) => (
                <div key={section.section || gi}>
                  {section.section && (
                    <h2 style={{ ...display, color: NEON }} className="mb-6 text-2xl font-extrabold uppercase tracking-[0.02em]">{section.section}</h2>
                  )}
                  {section.categories.map((catg) => (
                    <div key={catg.category ?? "_"} className={section.categories.length > 1 ? "mt-8" : ""}>
                      {catg.category && (
                        <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.24em]" style={{ color: CYAN }}>{catg.category}</p>
                      )}
