            ...
            <div className="space-y-16">
              {groups.map((section, gi) => (
                <div key={section.section || gi}>
                  {section.section && (
                    <h2 data-edit={`section:${section.categories[0]?.items[0]?.id ?? ""}`} style={{ ...display, color: BRASS }} className="mb-6 text-2xl font-bold uppercase tracking-[0.02em]">{section.section}</h2>
                  )}
