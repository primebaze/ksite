...
// Haven — calm, restorative wellness & recovery studio design (light, spa-like).
// MULTI-PAGE: nav opens real routes (Services / About / Gallery / Contact) under
// basePath, never scroll anchors. Sticky header + warm sand footer shared.
// Palette baked: warm sand / cream / sage with a serif voice; the tenant swaps
// in their own media, services, hours and copy.

const serif = { fontFamily: "var(--font-fraunces)" } as const;
const SAND = "#f6f1e9"; // warm page
const CREAM = "#fbf8f2"; // lighter panel
const PANEL = "#efe7da"; // tinted band
const INK = "#33302a"; // soft ink heading/body
const SAGE = "#7c8a6f"; // sage accent
const MUTE = "#857d70"; // muted body
const LINE = "#33302a1f"; // faint hairline
...
function Kicker({ children, center = false }: { children: ReactNode; center?: boolean }) {
  return (
    <p className={`flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.32em] ${center ? "justify-center" : ""}`} style={{ color: SAGE }}>
      <span className="h-px w-7" style={{ background: SAGE }} />
      {children}
    </p>
  );
}
...
export default function Haven({ site, page = "home", basePath = "" }: PresetProps) {
  ...
  const ctaLabel = content.cta_label || "Book a visit";
          ...
          {content.socials && content.socials.length > 0 && (
            <div className="mt-6 flex gap-3">
              {content.socials.map((s) => (
                <a key={s.url} href={s.url} target="_blank" rel="noreferrer" aria-label={s.label} className="grid h-9 w-9 place-items-center rounded-full transition hover:opacity-70" style={{ border: `1px solid ${SAGE}66`, color: SAGE }}><SocialIcon kind={`${s.label} ${s.url}`} /></a>
              ))}
            </div>
          )}
        </div>
  ...
  const shell = (children: ReactNode, solid = true) => (
    <div id="top" style={{ ...siteRootStyle(theme, tokens), background: SAND, color: INK }} className="min-h-screen font-body">
      <HavenHeader name={name} cta={ctaLabel} ctaHref={join} links={nav} home={href("home")} solid={solid} />
      {children}
      {footer}
    </div>
  );
            ...
            <div className="space-y-14">
              {groups.map((section, gi) => (
                <div key={section.section || gi}>
                  {section.section && (
                    <h2 data-edit={`section:${section.categories[0]?.items[0]?.id ?? ""}`} style={{ ...serif, color: INK }} className="mb-6 text-2xl">{section.section}</h2>
                  )}
                  {section.categories.map((catg) => (
                    <div key={catg.category ?? "_"} className={section.categories.length > 1 ? "mt-8" : ""}>
                      {catg.category && (
                        <p data-edit={`category:${catg.items[0]?.id ?? ""}`} className="mb-3 text-[11px] font-semibold uppercase tracking-[0.24em]" style={{ color: SAGE }}>{catg.category}</p>
                      )}
              ...
              <div className="text-center">
                <a href={join} className="inline-flex rounded-full px-9 py-3.5 text-[11px] font-semibold uppercase tracking-[0.22em] text-white transition hover:opacity-90" style={{ background: SAGE }}>{ctaLabel}</a>
              </div>
            </div>
          ) : <p className="text-center" style={{ color: MUTE }}>Our services are coming soon.</p>}
        </section>
      </>,
    );
  }
      ...
      <>
        ...
        <section className="mx-auto grid max-w-6xl gap-12 px-6 py-20 sm:px-8 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="space-y-5">
            {bookingOn && <FitnessBooking tenantId={tenant.id} name={name} skin={bookingSkin} classes={classNames} title="Book a visit" sub="Choose a treatment or class and we'll confirm your time by phone or email." />}
            {contactOn && (
              <SiteContactForms
                tenantId={tenant.id}
                booking={false}
                ...
                contactBlurb="Questions about treatments, memberships or first visits? We'll be in touch."
                contactCta="Send note"
                theme={{ card: CREAM, cardBorder: LINE, heading: INK, blurb: MUTE, label: "#6f6a5f", fieldBg: "#ffffff", fieldBorder: "#33302a26", fieldText: INK, button: SAGE, buttonText: "#ffffff", radius: "9999px", font: "var(--font-fraunces)" }}
              />
            )}
          </div>
        </section>
  ...
  return shell(
    <>
      {/* hero — calm, centered over imagery */}
      <section className="relative isolate flex min-h-[92vh] flex-col items-center justify-center overflow-hidden text-center">
        {video ? (
          ...
          <div className="absolute inset-0" style={{ background: "linear-gradient(160deg,#c9cbb6,#a7b196)" }} />
        )}
        <div className="pointer-events-none absolute inset-0" style={{ background: "linear-gradient(to top, rgba(40,38,32,0.6) 0%, rgba(40,38,32,0.25) 50%, rgba(40,38,32,0.35) 100%)" }} />
        <div className="relative z-10 mx-auto max-w-2xl px-6">
          <p className="text-[11px] font-semibold uppercase tracking-[0.36em] text-white/85">{content.tagline || "Rest · Recover · Restore"}</p>
          <h1 style={serif} className="mt-5 text-5xl font-medium leading-[1.05] text-white [text-shadow:0_2px_24px_rgba(0,0,0,0.45)] sm:text-7xl">
            <span data-edit="tenant.business_name">{name}</span>
          </h1>
          {content.tagline && <p data-edit="content.tagline" className="mx-auto mt-5 max-w-md text-[15px] leading-relaxed text-white/85 [text-shadow:0_2px_16px_rgba(0,0,0,0.4)]">{content.tagline}</p>}
      ...
      {/* services preview — clean divider row list */}
      {featured.length > 0 && (
