                    ...
                    <div key={section.section} className="break-inside-avoid">
                      {section.section && (
                        <div className="mb-6 text-center">
                          <span style={{ ...display, color: ROSE, fontStyle: "italic" }} className="text-3xl font-medium">{section.section}</span>
                          <div className="mx-auto mt-3 h-px w-20" style={{ background: `${GOLD}99` }} />
                        </div>
                      )}
                      {section.categories.map((catg) => (
                        <div key={catg.category ?? "_"} className="mb-8">
                          {catg.category && <p className="mb-3 text-center text-[11px] font-semibold uppercase tracking-[0.24em]" style={{ color: SAGE }}>{catg.category}</p>}
