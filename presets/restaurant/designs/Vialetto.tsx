                    ...
                    <div key={section.section} className="break-inside-avoid">
                      {section.section && (
                        <div className="mb-7 text-center">
                          <h2 data-edit={`section:${section.categories[0]?.items[0]?.id ?? ""}`} style={serif} className="text-3xl italic lowercase">{section.section}</h2>
                          <div className="mt-3"><Flourish /></div>
                        </div>
                      )}
                      {section.categories.map((catg) => (
                        <div key={catg.category ?? "_"} className="mb-9">
                          {catg.category && <p data-edit={`category:${catg.items[0]?.id ?? ""}`} className="mb-4 text-center text-[11px] font-semibold uppercase tracking-[0.28em]" style={{ color: TERRA }}>{catg.category}</p>}
