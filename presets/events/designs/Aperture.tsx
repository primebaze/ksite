          ...
          <section className="mx-auto max-w-6xl px-3 py-12 sm:px-6">
            <div className="columns-1 gap-3 sm:columns-2 lg:columns-3 [&>*]:mb-3">
              {gallery.map((g) => (
                ...
                <img loading="lazy" decoding="async" key={g.id} src={g.image_url} alt={g.caption ?? ""} className="w-full break-inside-avoid object-cover" />
              ))}
            </div>
          </section>
        ...
        <section style={{ borderTop: `1px solid ${LINE}` }} className="py-20">
          ...
          <div className="mx-auto mt-12 grid max-w-6xl grid-cols-2 gap-3 px-3 sm:grid-cols-3 sm:px-6">
            {gallery.slice(0, 6).map((g, i) => (
              // eslint-disable-next-line @next/next/no-img-element
              <img loading="lazy" decoding="async" key={g.id} src={g.image_url} alt={g.caption ?? ""} className={`w-full object-cover ${i === 0 ? "col-span-2 row-span-2 aspect-square sm:aspect-[4/5]" : "aspect-[4/5]"}`} />
            ))}
          </div>
        </section>
      )}
