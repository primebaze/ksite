...
// Consultation booking widget for the Lumiere beauty-clinic design. Posts to the
...
// "company"; sending / sent / error states.
export function LumiereBooking({ tenantId, name }: { tenantId: string; name: string }) {
  ...
  return (
    <div id="book" className="h-fit border border-neutral-200 bg-white p-8 shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
      <p style={{ fontFamily: "var(--font-fraunces)" }} className="text-2xl" >Request your appointment</p>
      <p className="mt-1.5 text-sm text-neutral-500">We will confirm your time by phone or email.</p>
      ...
      {status === "sent" ? (
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-5">
          ...
          <label className="block">
            ...
            <input name="party" className={fieldCls} placeholder="e.g. consultation, facial, hair colour" />
          </label>
          ...
          <label className="block">
            <span className="text-[11px] font-medium uppercase tracking-[0.18em] text-neutral-500">Anything else?</span>
          ...
          <button
            ...
            style={{ background: status === "sending" ? INK : GOLD }}
          >
