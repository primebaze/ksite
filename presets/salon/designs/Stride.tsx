                  ...
                  <div className="mt-6 grid gap-x-14 gap-y-10 sm:grid-cols-2">
                    {section.categories.map((catg) => (
                      <div key={catg.category ?? "_"}>
                        {catg.category && (
                          <h3 className="border-b pb-2 text-xs font-semibold uppercase tracking-[0.16em]" style={{ color: TEAL, borderColor: AQUA }}>{catg.category}</h3>
                        )}
      ...
      <section className="grid lg:grid-cols-2">
        <div className="relative min-h-[360px] overflow-hidden" style={{ background: AQUA }}>
          {video ? (
            ...
            // eslint-disable-next-line @next/next/no-img-element
            <img src={gallery[1]?.image_url || heroImg} alt="" className="absolute inset-0 h-full w-full object-cover" />
          ) : (
